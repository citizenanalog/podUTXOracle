...
def Ask_Node(command):
    # Convert any bytes in command to strings
    podman_command = ['sudo', 'podman', 'exec', '-it', 'bitcoind.embassy', 'bitcoin-cli'] + [
        arg.decode('utf-8') if isinstance(arg, bytes) else str(arg) for arg in command
    ]
    #print(f"Executing command: {' '.join(podman_command)}")  # Debug
    answer = None
    try:
        answer = subprocess.check_output(podman_command, stderr=subprocess.STDOUT)  # Capture stderr
        #print(f"Command output: {answer}")  # Debug
    except Exception as e:
        print("Error connecting to your node. Troubleshooting steps:\n")
        print("\t 1) Make sure podman is installed and working")
        print("\t 2) Make sure bitcoind.embassy container is running")
        print("\t 3) Verify sudo permissions for podman command")
        print("\nThe command was: "+" ".join(podman_command))
        print("\nThe error was:\n")
        print(e)
        sys.exit()
    return answer
...
#grant read access to the blocks directory
...
#
# â€œUTXOracle Localâ€ refers to the open-source software made available by the
...
#
# â€œThe Authorâ€ refers to the creator and copyright holder of UTXOracle, reachable
...
#    canonical names:
#    - â€œUTXOracle Consensus Priceâ€ for the 24-hour average from confirmed blocks
#    - â€œUTXOracle Block Window Priceâ€ for the average from the most recent 144
#      confirmed blocks
...
#    - â€œUTXOracle Consensus Priceâ€
#    - â€œUTXOracle Block Window Priceâ€
#    - Any term incorporating â€œUTXOracleâ€ to describe the price output
...
#    - â€œUTXOracle Liveâ€
#    - â€œUTXOracle Live Priceâ€
#    - â€œLive On-Chain Priceâ€
...
#
# This software is provided â€œas is,â€ without warranty of any kind. The Author shall
